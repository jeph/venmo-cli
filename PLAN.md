...
- Use Rust 1.97.1 for the pinned toolchain and Rust 1.97 for the MSRV, package version `0.3.0`, and the MIT license.
...
venmo pay user <USERNAME> <AMOUNT> <NOTE> [--source <SOURCE_ID>] [--protect] [--visibility <VISIBILITY>] [--yes | --dry-run]
venmo requests create <USERNAME> <AMOUNT> <NOTE> [--visibility <VISIBILITY>] [--yes | --dry-run]
...
If the initial payment submission receives HTTP 403 with root `error.title` exactly
`OTP_STEP_UP_REQUIRED`, treat it as
...
- Rejects payment-only options; `--dry-run` performs this command's full request-creation preflight and stops before F3.
- Normally performs one write and is never retried automatically. The exact P2P SMS challenge may
  add one verified continuation with the same client UUID.
...
This is a financial write that pays exactly one existing incoming request through one current-API immutable funding plan:
...
- Acceptance is unprotected by default. Every acceptance resolves exactly one unacknowledged request notification whose payment ID equals `REQUEST_ID`, then loads peer-eligible balance and external methods and applies the shared funding-source policy. A legacy-shaped direct notification uses root `id`/`payment.id`; the current wrapper uses `additional_properties.request.id`/`additional_properties.request.payment.id`. The write always uses the associated request-action ID—not the current wrapper's outer notification ID—and the selected source exactly. Missing, duplicate, conflicting, or malformed notification matches, unavailable explicit sources, and insufficient explicit balance fail before confirmation. Returned fee records are omitted from an unprotected approval.
- `--source` accepts one exact peer-eligible balance, bank, or card ID listed by `venmo pay options` and never silently substitutes another source.
- `--protect` explicitly turns on Purchase Protection and submits strictly validated fee records. The checked seller fee must not exceed the request amount.
...
- The sanitized selected source and, for an external source, its method-level fee evidence.
...
Signer-verified Android 10.31.1 and 26.13.0 establish the acceptance flow:
...
options for balance or external funding. A bounded client-1 read confirmed the current three-ID wrapper structure. An outer-wrapper
...
The exact HTTP-403/root-title SMS challenge uses the server UUID from root error metadata, one shared
hidden prompt, one verification, and at most one F4S continuation that preserves source/token/fees.
The actual debit source/final fee, protected branch, and live acceptance step-up remain unproven.
...
- The 2026-07-20 dependency refresh advanced the lock to `keyring` 4.1.5 with the same deliberately minimal `v1` feature policy.
...
- Maintained descendants support bearer-authenticated JSON `PUT /v1/payments/{id}` with `action: "cancel"` or `"remind"`, but their listing scope proves only outgoing/requester-owned requests. They do not establish incoming denial. Historical `approve` evidence is retained above for provenance; production acceptance has migrated entirely to the current request-action route. Incoming `deny` and outgoing `cancel` remain separate operation-specific contracts.
- Signer-verified Android 26.13.0 establishes protected peer payment independently from request acceptance: source-bound form eligibility has no `action` field; the app selects an applied fee whose product URI starts with `venmo:product:buyer_protection:`; protected F2 sends `transaction_type: goods_services_protected`, the complete singleton fee, and quasi-cash metadata; protected response records use the distinct `type: payment_protected`; and SMS step-up rebuilds the same protected transaction while adding verification metadata. The CLI implements those exact boundaries with stricter duplicate matching rejection and service-free tests. One owner-authorized, non-retried $1 validation on 2026-07-20 received eligibility 201 and payment 200 without OTP and reconciled to exactly one settled payment. A bounded read-only detail probe confirmed `type: payment_protected`. The initial validator had incorrectly required the request-side `goods_services_protected` value in the response and therefore conservatively reported the successful write as ambiguous; it was not retried. Final seller fee, tax treatment, item eligibility, and actual debit source remain unproven.
- Subsequent signer-verified static analysis of Android 10.31.1 and 26.13.0 established the current acceptance contract: unacknowledged notification lookup, exact payment-ID association to a request notification's own action ID, source-bound eligibility, and `PUT /v1/requests/{request-action-id}` options carrying `funding_source_id`, `eligibility_token`, bounded applied-fee records, and quasi-cash metadata. Android 26.13.0 additionally models current wrapper notifications under `additional_properties.request` and the exact server-UUID SMS-step-up branch. A separately approved bounded client-1 read confirmed the outer notification ID, nested request-action ID, and nested payment ID are distinct. Native code includes applied fees only when Purchase Protection is selected. The CLI uses this route for every acceptance, defaults unprotected, omits fees in that mode, and normalizes/submits them only for explicit `--protect`. An outer-wrapper-ID F4S attempt returned HTTP 404 and reconciled as no mutation. A corrected nested-action-ID acceptance returned 200 and reconciled as one matching settled $20 activity with the request removed. Actual source/final fee, protected approval, and live step-up continuation remain unproven.
...
| Runtime | `tokio` 1.53 | CLI keeps its current-thread runtime; MCP additionally needs reviewed stdio I/O, sync, and multi-thread runtime features |
...
Every future payment, request-creation, request-acceptance, and request-decline tool must be marked `readOnlyHint=false`, `destructiveHint=true`, `idempotentHint=false`, and `openWorldHint=true`. These hints help trusted hosts display policy and warnings but do not replace capability restriction or approval. The tool executes one initial write; the exact P2P SMS challenge may add at most one operation-specific verified continuation. It must not be retried by the server after timeout, cancellation, disconnect, protocol reconnect, or unknown outcome. P2P step-up remains unavailable to MCP until a trusted host can provide secret-safe human OTP input without exposing it to the model or protocol transcript.
...
- `--from` rejected by `pay user` and every other command; `--source` accepted only by `pay user`
  ...
  `requests create`, `requests accept`, `requests decline`, `requests cancel`, `friends add`,
  `friends remove`, and `transfer out`, and rejected by `pay options`, `friends list`, and request
  reads.
- `--dry-run` accepted only by the eight `--yes` mutation leaves, conflicts with `--yes`, and stops after full preflight/detail output without installing signal protection or writing.
...
- Every request acceptance resolves the request-action ID and uses exact shared source selection plus source-bound eligibility before confirmation. Unprotected acceptance omits returned fee records. Explicit `--protect` checks/displays/submits normalized fee records and rejects malformed, overflowing, or greater-than-request fees before any write. No legacy fallback is attempted.
...
- Review and lock the official `rmcp` stable release and minimal stdio/server/schema feature set against Rust 1.97, supported targets, and cargo-deny.
...
6. Whether controlled validation can establish protected external-funded acceptance, actual
   source/fee evidence, and live request-acceptance SMS-step-up success. Unprotected
...
8. The exact published official `rmcp` release and minimal feature set that satisfy Rust 1.97, macOS/Linux, schema, license, advisory, and cargo-deny requirements.

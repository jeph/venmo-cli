...
For `pay user`, automatic funding uses the peer-eligible Venmo balance source when authoritative available balance covers the full amount. Otherwise it chooses the unique default peer-eligible external bank or card, or the sole external method when no default exists. External ambiguity is evaluated only when an automatic external choice is needed; selection never uses response order or fee. `--source` accepts an exact peer-eligible balance, bank, or card ID from `venmo pay options`; explicit balance must cover the full amount, and unknown or ineligible IDs fail before confirmation. The CLI submits an explicitly requested source exactly and never silently substitutes another source. Payment details distinguish automatic from explicit selection, show the submitted source, show applicable method-level fee evidence, and report the separate eligibility fee. Venmo's response does not prove the actual debit source or final fee. `pay user` retains only the final default-No confirmation when both stdin and stderr are terminals; otherwise it requires `--yes`.

Venmo may reject the first payment submission with its exact SMS-OTP step-up response. The CLI supports that continuation only on an interactive terminal: it requests one SMS, reads one hidden six-digit code, verifies it, and submits the same immutable payment and UUID exactly once with Venmo's verified-SMS metadata. It does not loop or automatically retry. `--yes` skips the payment confirmation but cannot bypass an OTP challenge. The OTP is never accepted as an argument, printed, logged, or stored. Current headers and the complete challenge flow are pinned by Venmo Android 26.13.0 inspection, exact synthetic tests, and an owner-run live validation: one explicit-bank-source `$1.00` payment completed after OTP and independently reconciled as exactly one settled outgoing activity. That proves the submitted flow, not the actual debit source or final fee. Exact payment-creation HTTP 403/root code `1360` is reported as Venmo's 10-minute same-recipient/amount/note duplicate rejection; exact code `10100` reports that Venmo's server-side checks blocked the payment and tells the operator to try again later or use the official app.

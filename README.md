...
records. It shows at most the first five comments while still printing the server count and whether
the source collection is complete. When a complete collection contains more comments, it prints the
exact `activity comments list` command and offset for the remaining rows. The CLI never treats a
truncated embedding as complete.
...
Comment listing matches the other offset-based list defaults: `--limit 10`, `--offset 0`, and a
maximum limit of 50. Venmo currently supplies comments inside the single activity-detail response,
so the CLI validates that the entire embedded collection is present and then applies the requested
page locally in source order. It performs one story-detail read per invocation, never fetches hidden
pages, writes `Next offset: <N>` when more embedded rows remain, and fails closed rather than
claiming exhaustive listing if Venmo ever returns a partial collection.
...
Owner-approved reversible client-1 canaries on 2026-07-21 validated like, duplicate-like
preflight rejection, unlike, duplicate-unlike preflight rejection, self-authored comment addition,
and comment removal on private settled activities. A separate bounded pagination canary created 51
temporary comments on a consenting private activity; one detail response returned all 51 in source
order with exact count/data equality and no continuation. Every temporary comment was then removed
exactly once and the activity was verified back at zero comments. This establishes controlled
evidence for local pagination, not a dedicated comments-list endpoint or universal durability.
...
The CLI always prompts for the account identifier, password, and a newly supplied trusted device ID, followed—only if Venmo returns the recognized challenge—by SMS OTP. Password, device ID, and OTP input are hidden with terminal echo disabled. Login never reuses a stored device ID because Venmo may have invalidated it. It sends one non-retried authentication attempt, validates any issued token with the current-account endpoint, then stores only the validated bearer token, newly supplied device ID, and account metadata. Direct password success requires no redundant device-trust update; after OTP, device trust is attempted once after the validated credential is safely stored. Account identifiers, passwords, OTPs, and OTP secrets are never stored.
...
The CLI does not import bearer tokens, store reusable passwords, refresh sessions automatically, retry authentication, or expose a separate reauthentication command. A definitive HTTP 401 from an authenticated request preserves the local credential and directs the user to run `venmo auth login` explicitly. Arbitrary non-401 API rejections retain their general classification, and uncertain financial writes remain ambiguous and must not be retried.

Never manually copy a device ID, password, OTP, bearer token, or browser cookie into a command argument, environment variable, arbitrary file, chat, screenshot, or log. The managed Linux XDG fallback documented below is the only file-persistence exception. Login exposes no secret, environment, or stdin-file flags. If Venmo rejects an attempt, stop rather than repeatedly retrying.

#### Local credential storage

macOS and Windows always use the native OS keyring. On Linux, each credential-backed command first queries the user D-Bus for `org.freedesktop.secrets`. A running or D-Bus-activatable Secret Service always selects the keyring; locked, denied, misconfigured, or otherwise failing keyring access never downgrades to a file. A reachable D-Bus that advertises no Secret Service selects the fallback. An absent session-bus endpoint also selects the fallback so genuinely headless systems can persist a session; malformed addresses, permission failures, protocol errors, query failures, and timeouts fail closed.

The Linux fallback is `$XDG_STATE_HOME/venmo-cli/credential.json`, or `$HOME/.local/state/venmo-cli/credential.json` when `XDG_STATE_HOME` is unset. It is a plaintext JSON bearer-token envelope protected by an owner-only directory and file (`0700` and `0600`), current-user ownership checks, no-follow opens, hard-link rejection, bounded reads, and atomic replacement. It protects against other unprivileged local users, but not root, malware running as the same user, filesystem backups, or offline access to the account. Fallback login displays this warning and requires an explicit default-No confirmation before contacting Venmo.

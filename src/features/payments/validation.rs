...
use crate::shared::{Account, ApplicationFailureKind, CredentialEnvelope};
...
impl FinancialValidationError {
    #[must_use]
    pub const fn failure_kind(self) -> ApplicationFailureKind {
        match self {
            Self::AccountMismatch => ApplicationFailureKind::Credential,
            Self::SelfRecipient | Self::UnsupportedProfileType | Self::RecipientNotPayable => {
                ApplicationFailureKind::Usage
            }
            Self::MissingProfileType | Self::MissingPayability => {

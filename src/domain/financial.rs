...
use super::{
    Account, Balance, Money, Note, PaymentId, PaymentMethod, PaymentMethodId, PendingRequest,
    RequestId, RequestStatus, User,
};
...
impl AcceptRequestPlan {
    #[must_use]
    pub(crate) const fn new(account: Account, request: PendingRequest, balance: Balance) -> Self {
        Self {
            account,
            request,
            balance,
...
impl DeclineRequestPlan {
    #[must_use]
    pub(crate) const fn new(account: Account, request: PendingRequest) -> Self {

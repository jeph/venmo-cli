    ...
    fn record_write_once(&mut self) {
        if !self.recorded_write {
            ...
            self.transcript.borrow_mut().push(match self.stream {
                Stream::Stdout => PayCall::StdoutWrite,
                Stream::Stderr => PayCall::StderrWrite,
            });
        }
    }
}
...
impl Write for OrderedWriter {
    fn write(&mut self, buffer: &[u8]) -> io::Result<usize> {
        ...
        let text = std::str::from_utf8(buffer).map_err(io::Error::other)?;
    ...
    fn flush(&mut self) -> io::Result<()> {
        self.transcript.borrow_mut().push(match self.stream {
            Stream::Stdout => PayCall::StdoutFlush,
            Stream::Stderr => PayCall::StderrFlush,
        });

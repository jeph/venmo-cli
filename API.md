...
> run a real `pay`, `request`, `accept`, `decline`, or transfer as a test. Use the
...
|---:|---|---|---|
| 1 | `auth login [--token]` | Import: **A5**. Direct password: **A1 → A5**. OTP: **A1 → A2 → A3 → A5**, verified local save, then **A4** (maximum 5). | Remote auth plus hidden prompts/keyring. |
| 2 | `auth reauthenticate` | Direct: **A1 → A5**. OTP: **A1 → A2 → A3 → A5**, save, then **A4** (maximum 5). | Full issuance, not refresh. |
| 3 | `auth logout [--revoke]` | No revoke: no call. With readable credential and `--revoke`: **A6** once; local deletion is attempted regardless. | Primarily local. |
| 4 | `auth status` | **A5** once. | Live identity validation. |
| 5 | `pay <RECIPIENT> <AMOUNT> --note <NOTE> [--visibility ...] [--yes]` | **A5**; **P2**, or 1–4 × **P1** then **P2**; **W2 → W1 → F1 → F2**. Numeric maximum 6; username maximum 10. | Financial; exactly one **F2**. |
| 6 | `request <RECIPIENT> <AMOUNT> --note <NOTE> [--visibility ...]` | **A5**; recipient resolution; **F3**. Numeric 3; username maximum 7. | Financial; exactly one **F3**; no prompt. |
| 7 | `accept <REQUEST_ID> [--yes]` | **A5 → R2 → P2 → W2 → F4** (5). | Financial; exactly one **F4**. |
| 8 | `decline <REQUEST_ID>` | **A5 → R2 → F5** (3). | State-changing; exactly one **F5**; no prompt. |
| 9 | `friends list [--limit N] [--offset N]` | **P3** once. | One page; stored self ID. |
| 10 | `users search <QUERY> [--limit N] [--offset N]` | **P1** once. | One page. |
| 11 | `users info <USER_ID>` | **P2** once. | One detail read. |
| 12 | `payment-methods list` | **W1** once. | One read. |
| 13 | `balance` | **W2** once. | One read. |
| 14 | `activity list [--limit N] [--before-id TOKEN]` | **AC1** once. | One page; stored self ID. |
| 15 | `activity info <ACTIVITY_ID>` | **AC2** once. | One detail read. |
| 16 | `requests list [--direction all\|incoming\|outgoing] [--limit N] [--before TOKEN]` | **R1** once; direction is filtered locally. | One unfiltered source page. |
| 17 | `requests info <REQUEST_ID>` | **R2** once; narrowed locally to open requests. | One detail read. |
| 18 | `transfer options` | **T1** once. | Enabled read-only current eligibility view. |
| 19 | `transfer out <AMOUNT> --speed standard [--yes]` | **A5 → W2 → T1 → T2** (4). | Financial; exactly one **T2** after default-No confirmation. |
| 20 | `doctor` | **D1**; if credential readable, **A5**; after identity match, **W2 → W1 → P3(limit=1,offset=0) → AC1(limit=1) → R1(limit=1)**. Healthy path 7. | Read-only diagnostics. |
| 21 | `completions bash\|zsh\|fish\|powershell` | No network, keyring, API construction, or logging setup. | Entirely local. |
...
transfer-in command, instant transfer write, or manual transfer-destination selection.
...
- A JSON body additionally sends `Content-Type: application/json`.
...
| Authenticated | `Authorization: Bearer <ACCESS_TOKEN>`, `device-id: <DEVICE_ID>` | **A4–A6** and all other mobile operations |
...
- **Recipient:** such an ID or a username beginning with `@`. The parser removes
  exactly the first prefix; the remaining 1–1,023 bytes have no whitespace/control
  characters but may contain or begin with `@` (so `@@alice` is accepted).
- **Search query:** nonblank, at most 1,024 bytes, no controls. An `@` query loses
  its first prefix on wire and adds `type=username`.
...
#### A6 — revoke current token
```http
DELETE /v1/oauth/access_token
```

No body; generic success permits an empty 2xx body. Current live revocation behavior
remains an evidence gap; local logout does not depend on it succeeding.
...
### 5.4 Doctor connectivity
#### D1 — bounded unauthenticated reachability
```http
GET /v1/account
```

No auth/query/body. Any fully captured bounded nonredirect response passes regardless
of HTTP status or JSON. It proves DNS/TLS/HTTP reachability only; A5 and later shape
probes separately establish authentication and schema health.
...
password/OTP values are zeroized and not stored. Login/reauthentication needs a
terminal, and every save is reread and checked for v1 equivalence.

- Token import reuses a stored device ID or hidden-prompts for its match, runs A5,
  rejects a different stored account, and saves.
- Password login will not overwrite readable credentials. Direct A1 runs A5, saves,
  and skips A4. OTP runs A2/A3, A5, save, then A4; A4 failure reports incomplete
  trust but leaves the validated credential stored.
- Reauthentication reuses the device ID without validating/revoking the old token,
  performs full issuance (not refresh), and replaces only after matching A5 ID.
- Logout without `--revoke` deletes locally. With it, readable state triggers A6,
  then deletion regardless; both outcomes are reported. Missing state succeeds.
...
audience. The CLI renders and flushes authoritative preflight, installs interruption
protection, then attempts exactly one F5 without a prompt or `--yes`. No P2/W1/W2/F1
runs, profile type/payability is unproved, and no money/funding field is sent.
### 6.3 Reads and doctor
...
have a continuation. Doctor runs D1 independently, requires A5 identity match before
its five authenticated shape probes, and continues collecting independent failures.
...
Financial `@username` resolution is the only automatic pagination: P1 uses limit 50
...
ambiguous. No match at the bound is incomplete, not authoritative absence. One match
at the bound is usable only after P2 returns the same ID and username.

## 8. No-retry and ambiguous-outcome policy
F2–F5 and T2 are enabled financial-policy writes. A1/A3 use a parallel but distinct
authentication issuance ambiguity.
...
successful result to the operator. Usage exits 2, ordinary failure/cancellation 1,
...
| 2026-07-11 | A5/token import | Direct observation / controlled live validation | Matching token/device, A5 identity, and keyring save/reread. |
| 2026-07-11 | A1 direct login/reauth | Direct observation / controlled live validation | Trusted-device direct issuance and stored-device renewal; redundant A4 was rejected and removed. |
...
| 2026-07-11 | Reads/story classes | Direct observation / controlled live validation | A5, W1/W2, P1–P3, AC1/AC2, R1/R2, all activity classes, doctor, and read smoke. Settled payment-ID detail returned HTTP 500. |
...
| Current gap | A6 | Existing behavior + historical lead / exact synthetic; residual gap | Local semantics are exact; remote live revocation is not current-proved. |
...
Inbound, instant, debit, server minimum/maximum interpretation, step-up, cancellation,
and expedition remain independently gated. Standard response net/fee cents are
...
| Token lifecycle/A6 | Issuance, local storage/logout, synthetic revoke. | Refresh/lifetime/current remote revocation. |
...
| Transfers | Current T1 options read; enabled fail-closed T2 standard-bank write; exact HTTP-201 pending response; matching-ID activity reconciliation. | Settlement completion, confirmed rejection codes, inbound, instant, debit, T1 fee/limit units, step-up, cancellation, or expedition. |
...
7. Keep financial writes one-shot: never replay after an ambiguous transmission. A
